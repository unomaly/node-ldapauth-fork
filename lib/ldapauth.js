...
function LdapAuth(opts) {
  ...
  if (opts.reconnect) {
    ...
    this.once('_installReconnectListener', function() {
      self.log && self.log.info('install reconnect listener');
...
LdapAuth.prototype._handleError = function(err) {
  this.log && this.log.info('emitted error: %s', err);
  this._adminBound = false;
  this.emit('error', err);
};
...
LdapAuth.prototype._onConnectAdmin = function(callback) {
  ...
  self.log && self.log.info('_onConnectAdmin: %s', self.bindDN);
  self._adminClient.bind(
    self.bindDN,
    self.bindCredentials,
    function(err) {
      if (err) {
        self.log && self.log.info('_onConnectAdmin: admin bind error: %s', err);
        self._adminBound = false;
        return callback ? callback(err) : null;
      } else {
        self.log && self.log.info('_onConnectAdmin: admin bind ok');
      }
      self._adminBound = true;
...
LdapAuth.prototype._adminBind = function(callback) {
  if (this._adminBound) {
    var self = this;
    self.log && self.log.info('_onConnectAdmin: admin already bound');
    return callback();
  }
...
LdapAuth.prototype._search = function(searchBase, options, callback) {
  ...
  self.log && self.log.info('_search: %s', searchBase);
  self._adminBind(function(bindErr) {
    if (bindErr) {
      self.log && self.log.info('_search: admin bind error: %s', bindErr);
      return callback(bindErr);
    } else {
      self.log && self.log.info('_search: admin bind ok');
    }

    var emt = new EventEmitter();

    var items = [];
    emt.on('searchEntry', function(entry) {
      self.log && self.log.info('_search: admin search hit %s', entry);
    ...
    emt.on('error', callback);

    emt.on('end', function(result) {
      if (result.status !== 0) {
        var err = 'non-zero status from LDAP search: ' + result.status;
        self.log && self.log.info('_search: admin search end bad status %s', result.status);
        return callback(err);
      }
      self.log && self.log.info('_search: admin search returning %d items', items.length);
      return callback(null, items);
    });

    self.log && self.log.info('_search: admin search');
    self._adminClient.search(emt, searchBase, options, function(searchErr) {
      if (searchErr) {
        self.log && self.log.info('_search: admin search error: %s', searchErr);
        return callback(searchErr);
      } else {
        self.log && self.log.info('_search: admin search ok');
      }
    });
  });
};
...
LdapAuth.prototype._findUser = function(username, callback) {
  ...
  self.log && self.log.info('_findUser: %s', username);
  self._search(self.opts.searchBase, opts, function(err, result) {
    if (err) {
      self.log && self.log.info('_findUser: user search error');
      self.log && self.log.info('\t code: %s', err.code);
      self.log && self.log.info('\t name: %s', err.name);
      self.log && self.log.info('\t message: %s', err.message);
      return callback(err);
    } else {
      self.log && self.log.info('_findUser: user found');
    }
...
LdapAuth.prototype._findGroups = function(user, callback) {
  ...
  self.log && self.log.info('_findGroups: group search %s', searchFilter);
  self._search(self.opts.groupSearchBase, opts, function(err, result) {
    if (err) {
      self.log && self.log.info('_findGroups: group search error');
      self.log && self.log.info('\t code: %s', err.code);
      self.log && self.log.info('\t name: %s', err.name);
      self.log && self.log.info('\t message: %s', err.message);
      return callback(err);
    } else {
      self.log && self.log.info('_findGroups: group search ok');
    }
...
LdapAuth.prototype.authenticate = function(username, password, callback) {
  ...
  self.log && self.log.info('toplevel start');
  ...
  if (self.opts.cache) {
    ...
    if (cached && bcrypt.compareSync(password, cached.password)) {
      self.log && self.log.info('toplevel: provide from cache');
      return callback(null, cached.user);
    }
  }

  // 1. Find the user DN in question.
  self.log && self.log.info('toplevel: find user DN');
  self._findUser(username, function(findErr, user) {
    if (findErr) {
      self.log && self.log.info('toplevel: find user DN failed');
      return callback(findErr);
    } else if (!user) {
      self.log && self.log.info('toplevel: find user DN failed, no such user');
      return callback(format('no such user: "%s"', username));
    } else {
      self.log && self.log.info('toplevel: find user DN OK');
    }

    // 2. Attempt to bind as that user to check password.
    self.log && self.log.info('toplevel: bind for user');
    self._userClient.bind(user[self.opts.bindProperty], password, function(bindErr) {
      if (bindErr) {
        self.log && self.log.info('toplevel: bind user error: %s', bindErr);
        return callback(bindErr);
      } else {
        self.log && self.log.info('toplevel: bind user ok');
      }
      // 3. If requested, fetch user groups
      self.log && self.log.info('toplevel: get groups');
      self._getGroups(user, function(groupErr, userWithGroups) {
        if (groupErr) {
          self.log && self.log.info('toplevel: group search error %s', groupErr);
          return callback(groupErr);
        } else {
          self.log && self.log.info('toplevel: group search ok');
        }
        if (self.opts.cache) {
          self.log && self.log.info('toplevel: bcrypt hashing');
          bcrypt.hash(password, self._salt, function(err, hash) {
            if (err) {
              self.log && self.log.info('toplevel: bcrypt error, not caching %s', err);
            } else {
              self.log && self.log.info('toplevel: bcrypt ok');
              self.userCache.set(username, { password: hash, user: userWithGroups });
            }
            self.log && self.log.info('toplevel: all ok');
